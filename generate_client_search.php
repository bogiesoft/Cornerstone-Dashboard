<?php
	...
	if(isset($_POST["id_name_info"])){
		...
		if($result->num_rows > 0){
			...
			$arr_info = array($row["contact_name"], $row["phone"], $row["email1"], $row["address_line_1"], $row["city"], $row["state"], $row["zipcode"], $row["second_contact"], $row["fax"], $row["client_id"], $row["non_profit"], $row["crid"], $row["email2"], $row["address_line_2"]);
		}

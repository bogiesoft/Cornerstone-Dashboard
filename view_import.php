   ...
   //====================create datatable==========================================
  	dataTable = $('#crm-table').DataTable( {
        ...
        "deferRender": false,
  			"scrollX": true,
  			"ajax":{
  				url :"server-side-import.php?id=" + id, // json datasource
          ...
          data: function ( d ) {
           ...
           'render': function (data, type, row){
             if (row[0] == 1) {
               return '<input type="checkbox" id = "check_box" name="id[]" onclick = "rowMarkedClick(\'' + row[0] + '\',\'' + row[1] + '\',\'' + row[3] + '\')" checked>';
             }else {
               return '<input type="checkbox" id = "check_box" name="id[]" onclick = "rowMarkedClick(\'' + row[0] + '\',\'' + row[1] + '\',\'' + row[3] + '\')">';
             }
           }
         }
      ]
...
//buttons above table
  $("div.toolbar").html('<div class="dt-buttons"><a href="#" class="dt-button csv1"  id ="export" role="button">Export CSV</a></div>')
  //==============================================================================
...
<div class="actions">
<a id="back" href="uploadForm.php" class="action">Back To Importer</a>
</div>
<table id="crm-table"  cellpadding="0" cellspacing="0" border="0" class="display" width="100%">
        ...
        <tr>
          <td></td>
				<td><input type="text" text = "full_name" data-column="1"  placeholder = "Search Client Name" class="search-input-text search_col search_box0" style = "visibility: hidden"><button style = 'display: none' class = 'minus_button0' onclick = "minusSearchCounter('.search_box0', '.add_button0', '.minus_button0')">Clear Status</button><button class = "add_button0" onclick = "addSearchCounter('.search_box0', '.add_button0', '.minus_button0')">Search by Status</button></td>

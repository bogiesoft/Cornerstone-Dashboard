...
while(TRUE){
	if(isset($_POST[$id . $count])){
		...
		mysqli_query($conn, "UPDATE blue_sheet SET initialrec_count = '$initialrec_count', manual = '$manual', uncorrected = '$uncorrected', unverifiable = '$unverifiable', completed_date = '$completed_date', data_hrs = '$data_hrs', gd_hrs = '$gd_hrs', bs_foreigns = '$bs_foreigns', bs_exact = '$bs_exact', loose = '$loose', householded = '$householded', basic = '$basic', ncoa_errors = '$ncoa_errors', bs_ncoa = '$bs_ncoa', final_count = '$final_count', bs_domestic = '$bs_domestic' WHERE job_id = '$job_id'");
		break;
	}
	else{
		$count = $count + 1;
